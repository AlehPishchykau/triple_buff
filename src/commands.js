...
- Мат — часть стиля, используй свободно, но грамотно. Не коверкай слова ради мата
- Используй правильную дота-терминологию (названия героев, скиллов, итемов)
- Не выдумывай слова и не коверкай названия
...
- Если вопрос про всех игроков — вызови функцию для каждого.` },
	...
	const step2 = await client.chat.completions.create({
		...
		messages: [
			...messages,
			{ role: 'system', content: 'Ответь на вопрос по полученным данным. Грамотный русский, живо и с юмором. Мат к месту, не в каждом слове. Правильная дота-терминология. Кратко и по делу. Plain text без форматирования.' }
		],
	});
