...
ПРАВИЛА:
- Отвечай ТОЛЬКО на основе данных из функций. Не выдумывай факты.
- Если вопрос не про этих игроков или не про Dota 2 — ответь "Хз, братан, это не по моей части."
- Никогда не задавай уточняющих вопросов. Либо отвечай, либо говори что не знаешь.
- Если вопрос про всех игроков — вызови функцию для каждого.
- Вызывай только то, что нужно для ответа.` },
	...
	const step2 = await client.chat.completions.create({
		...
		messages: [
			...messages,
			{ role: 'system', content: 'Ответь на вопрос пользователя ТОЛЬКО по полученным данным. Кратко, по делу, с юмором. Не выдумывай. Не задавай вопросов. Если данных нет — скажи что не знаешь. Пиши plain text без форматирования.' }
		],
	});
